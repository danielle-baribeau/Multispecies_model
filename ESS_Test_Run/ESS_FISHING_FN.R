...
mn.prop.stock.fg.bm$bm.stock <- mn.prop.stock.fg.bm$bm.stock/10
mn.prop.stock.fg.bm$bm.bfg <- mn.prop.stock.fg.bm$bm.bfg/10
...
#no predict lambda-prop.bm relationship 
p.lambdas <- lambdas
...
#now start removals loop: and exploitation-biomass relationship (for removals)
rem.tmp <- NULL
s.count <- 0
for (d in c(L.stocks, M.stocks)){
  s.count <- s.count+1
  s.lam <- p.lambdas[p.lambdas$stock == d,]
  med.bm <- median(s.lam$pop.bm)
  #find RPs
  lrp <- 0.4*med.bm
  usr <- 0.8*med.bm
  #add RPs to management plan data frame
  mgmt.scen$low.threshold[s.count] <- lrp
  mgmt.scen$high.threshold[s.count] <- usr
...
for (i in 1:n.sims){
  ...
  prop.sim.K.L <-inv.logit(arima.sim(model =list(ar = c(L.com.prop.cor.lag.1)),n = n.yrs.proj + 1,
                                     ...
                                     innov = c(0,rnorm(n.yrs.proj,0,sd.L.sim.logit))) + med.L.sim.logit)[2:(n.yrs.proj + 1)]
  ...
  for (t in 1:n.yrs.proj){
    ...
    for(s in L.stocks)
    {
      ...
      if (bm.last.yr <= cur.K){
        ...
        if (bm.p >= 0.1) while(lam.samp > 1.5*max(stock.lambdas$lambda) || lam.samp < 0.9*min(stock.lambdas$lambda)) lam.samp <- rlnorm(1, log(lam.mn), lam.sd)
        bm.p <- NULL
      }
        ...
        
      {
        ...
        if (lam.mn < 0.5) while(lam.samp > mean(stock.lambdas$lambda) | lam.samp <  ((0.5*(min(stock.lambdas$lambda))))) lam.samp <- rlnorm(1,log(lam.mn*0.8),lam.sd)
    ...
    for(s in M.stocks)
    {
      ...
      if(bm.last.yr > cur.K) 
      {
        ...
        ## add restrictions on lambda by adding || statements here ##
        if(lam.mn > 0.5) while(lam.samp > mean(stock.lambdas$lambda) | lam.samp < ((0.9*(min(stock.lambdas$lambda))))) lam.samp <- rlnorm(1,log(lam.mn),lam.sd)
        if(lam.mn < 0.5) while(lam.samp > (0.5*mean(stock.lambdas$lambda)) | lam.samp < (0.5*(min(stock.lambdas$lambda)))) lam.samp <- rlnorm(1,log(0.5),lam.sd)  #while(lam.samp > median(stock.lambdas$lambda) || lam.samp < 0.5*min(stock.lambdas$lambda)) lam.samp <- rlnorm(1,log(lam.mn),lam.sd)
        #while(lam.samp < 0.1 || lam.samp > 10) lam.samp <- rlnorm(1,log(lam.med),lam.sd)
        #while (lam.samp < (min(stock.lambdas$lambda)) || lam.samp > 1.5*(max(stock.lambdas$lambda))) lam.samp <- rlnorm(1,log(lam.med),lam.sd)
      }
      #while(is.na(lam.samp)) lam.samp <- rlnorm(1,log(lam.mn),lam.sd)
...
#[ & res.sim.df$sim == 11,]
ggplot(na.omit(res.sim.df[res.sim.df$fg == "L" & res.sim.df$sim == 29 & res.sim.df$year != 2017,])) +
...
ggplot(na.omit(res.sim.df[res.sim.df$fg == "M"& res.sim.df$year != 2017 & res.sim.df$sim == 2,])) +
  geom_line(aes(x = year, y = stock.K/1000000, group = sim), colour = "lightblue") +
  geom_line(aes(x = year, y = bm.stock/1000000, group = sim)) +
