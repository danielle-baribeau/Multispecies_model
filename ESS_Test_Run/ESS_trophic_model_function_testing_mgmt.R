...
#add in trophic category 
stocks$troph.cat <- floor(stocks$TL)
#round up from 0.5
#for (i in 1:nrow(stocks)){
  #browser()
  if (stocks$TL[i] >= 3.5 && stocks$TL[i] < 4){
    stocks$troph.cat[i] = stocks$troph.cat[i] + 1
  }
  if(stocks$TL[i] >= 4.5){
    stocks$troph.cat[i] = stocks$troph.cat[i] + 1
  }
}
#Tried this, but made it so that TC 3 only made up ~0.01% of total ecosystem biomass
#Run into same issue with just 2 TCs - there is only ~0.01% in 3

bm.tst <- stocks
colnames(bm.tst)[colnames(bm.tst) == "code"] <- "Code"

bm.tot <- bm.tst

# The 'ecosystem' biomass and numbers
eco.tot.bm <- bm.tot |> collapse::fgroup_by(year) |> 
                    collapse::fsummarize(bm.eco = sum(pop.bm))
# Trophic level biomass and numbers.
trophic.bm <- bm.tot |> collapse::fgroup_by(year,troph.cat) |> 
                    collapse::fsummarize(bm.tl = sum(pop.bm))
...
# look at what the stock does within its TL.
...
bm.final <- bm.final[,-4]
names(bm.final) <- c("Code","Year","bm.stock", "common","trophic","troph.cat","bm.tl",'bm.eco',
                     'prop.bm.tl')
...
what.year <- bm.final |> collapse::fgroup_by(Code) |> collapse::fsummarize(min = min(Year),
                                                                      max = max(Year))
# The years we have data for all stocks
first.year <- max(what.year$min)
last.year <- min(what.year$max)
#removing 0s doesn't make us lose any years of the time series
...
yrs <- seq(2000,2024)
bm.best <- bm.best |> collapse::fsubset(Year %in% yrs)
...
# add some colors...
bm.best$color <- "black"
bm.best$color[bm.best$common %in% c("Witch flounder", "Longfin hake", "Smooth skate", "Longhorn sculpin",
                                    "Atlantic wolffish")] <- "blue"
bm.best$color[bm.best$common %in% c("Atlantic cod", "Haddock", "White hake", "Silver hake", "Pollock", 
                                    "Redfish","American plaice","Thorny skate",
                                    "Spiny dogfish", "Sea raven","Monkfish")] <- "orange"
#3 and 4 colours
bm.best$color[bm.best$common %in% c("Witch flounder", "Atlantic cod")] <- "blue"
bm.best$color[bm.best$common %in% c("Longfin hake", "Haddock")] <- "orange"
bm.best$color[bm.best$common %in% c("Smooth skate", "White hake")] <- "purple"
bm.best$color[bm.best$common %in% c("Longhorn sculpin", "Silver hake")] <- "grey"
bm.best$color[bm.best$common %in% c("Pollock", "Atlantic wolffish")] <- "lightblue"
bm.best$color[bm.best$common %in% c("Redfish")] <- "black"
bm.best$color[bm.best$common %in% c("American plaice")] <- "brown"
bm.best$color[bm.best$common %in% c("Thorny skate")] <- "darkorange"
bm.best$color[bm.best$common %in% c("Spiny dogfish")] <- "darkblue"
bm.best$color[bm.best$common %in% c("Sea raven")] <- "darkgrey"
bm.best$color[bm.best$common %in% c("Monkfish")] <- "darkorchid4"

# Put in Species + trophic level
bm.best$spec.tl <- paste(bm.best$common,"(TL = ",bm.best$trophic,")")
# Pull out meta data
meta.dat <- bm.best |> dplyr::group_by(Code,trophic,common,troph.cat,color,spec.tl) |> filter(row_number() >= (n() ))
meta.dat <- meta.dat[,c("Code","trophic","common","troph.cat","color","spec.tl")]
...
# Another color thing
colors2 <- distinct(bm.best, common, color)
pal2 <- colors$color
names(pal2) <- colors$common



stock.prop.bm.plt <- ggplot(bm.best) + geom_line(aes(x=Year,y=prop.bm.stock.tl,group = Code,color=spec.tl),linewidth=2) + 
                  facet_wrap(~troph.cat) + guides(colour = guide_legend(nrow = 6)) + theme(legend.position = 'top') +
                  scale_y_log10(name= "Proportion of biomass",n.breaks=10) + 
  scale_x_continuous(name="",labels = c(1983,1990,2000,2010, 2020),breaks=c(1983,1990,2000,2010,2020))+
                  scale_color_manual(values=pal)
save_plot(paste0(repo.loc,"/Figures/Prop_Biomass_ns_by_stock.png"),stock.prop.bm.plt,base_height = 8,base_width = 15)

stock.bm.plt <- ggplot(bm.best) + geom_line(aes(x=Year,y=bm.stock,group = Code,color=spec.tl),linewidth=2) + 
                     facet_wrap(~troph.cat) + scale_x_continuous(name="",labels = c(1983,1990,2000,2010, 2020),breaks=c(1983,1990,2000,2010, 2020))+
                     scale_y_log10(name = "Biomass",n.breaks=7) + theme(legend.position = 'top') +
                     guides(colour = guide_legend(nrow = 6)) + scale_color_manual(values=pal)
...
for(i in 1:n.sims) 
{
  ...
  {
    tl.stocks <- unique(bm.best$Code[bm.best$troph.cat==tl])
    ...
    for(s in tl.stocks)
    {
      ...
      {
        tmp.dat <- bm.best[bm.best$Code ==s,]
      ...
      {
        sim.Ks[[s]] <-  data.frame(Years = 1:n.yrs.proj, sim=i,
                                   Code = s, troph.cat = as.numeric(tl),
...
sim.stock.K.plt <- ggplot(sim.K.stocks[sim.K.stocks$sim==1,]) + geom_line(aes(x=Years,y=bm.stock,group=as.character(Code), 
  colour = as.character(Code)),linewidth=2) + 
...
############### Section 4 Multi-species model of ESS ############### 
...
# Take the biomass data for the ESS and subset it to the years we have data
bm.mod.yrs <- bm.tst |> collapse::fsubset(year %in% first.year:last.year)
for(s in stock.eco)
bm.start.year <- bm.mod.yrs |> collapse::fsubset(year == last.year) |> 
                         collapse::fgroup_by(code,TL,troph.cat,common) |> 
                         collapse::fsummarise(bm.tot = sum(pop.bm,na.rm=T))
...
#find median historic biomass values for all stocks
  ...
  #find median
    ...
    bm.best.stock <- bm.best |> collapse::fsubset(Code == s)
    #find median
...
## Finding equation of the line between low and high thresholds for each stock ##
  ...
  #find eqn components
    for (s in stock.eco) {
      ...
      thresholds <- data.frame(u = c(0, 0.4), bm = c(min.threshold[count.eqn], max.threshold[count.eqn]))
      #run model
...
for(j in 1:n.sims)
{
  ...
  for(s in stock.eco)
  {
    ...
    count.stock <- 1
      ...
      tmp.bm.last <- stock.bm.last |> collapse::fsubset(code == s)
      tmp.stock.K <- base.stock.K.tmp |> collapse::fsubset(Code == s)
      bm.ts.stock <- bm.final[bm.final$Code == s & bm.final$Year %in% first.year:last.year,]  
      #a=s
      #browser()
      ...
      { 
        ...
        res.ts[[s]] <- data.frame(net.bm = bm.start,removals = NA, mgmt.update = NA,
                                  ...
                                  troph.cat = as.numeric(unique(bm.best$troph.cat[bm.best$Code ==s])),
                                  K.bm = NA)
...
#ggplot(ts.unpack[[j]]) + geom_line(aes(x= Years,y=abund,group=Stock,color=Stock)) + facet_wrap(~troph.cat) + scale_y_log10()
  ...
  mgmt.scen$ex.curr <- c(rep(0.1, length(stock.eco)))
