...
#                 with each stocks being a names list, with the name of the list being a unique stock name.
...
trophic.bm <- bm.tot |> collapse::fgroup_by(Year,troph.cat) |> 
                    collapse::fsummarize(bm.tl = sum(bm))
...
names(bm.final) <- c("Stock","Year","trophic","species","troph.cat","bm.stock","bm.tl",'bm.eco',
                     'prop.bm.tl')
...
bm.best$color <- "black"
#3 and 4 colours
bm.best$color[bm.best$species %in% c("Witch flounder", "Atlantic cod")] <- "blue"
bm.best$color[bm.best$species %in% c("Longfin hake", "Haddock")] <- "orange"
bm.best$color[bm.best$species %in% c("Smooth skate", "White hake")] <- "purple"
bm.best$color[bm.best$species %in% c("Longhorn sculpin", "Silver hake")] <- "grey"
bm.best$color[bm.best$species %in% c("Pollock", "Atlantic wolffish")] <- "lightblue"
bm.best$color[bm.best$species %in% c("Redfish")] <- "black"
bm.best$color[bm.best$species %in% c("American plaice")] <- "brown"
bm.best$color[bm.best$species %in% c("Thorny skate")] <- "darkorange"
bm.best$color[bm.best$species %in% c("Spiny dogfish")] <- "darkblue"
bm.best$color[bm.best$species %in% c("Sea raven")] <- "darkgrey"
bm.best$color[bm.best$species %in% c("Monkfish")] <- "darkorchid4"
...
stock.prop.bm.plt <- ggplot(bm.best) + geom_line(aes(x=Year,y=prop.bm.stock.tl,group = Stock,color=spec.tl),linewidth=2) + 
                  ...
                  scale_y_log10(name= "Proportion of biomass",n.breaks=10) + scale_x_continuous(name="",labels = c(2000,2010,2020),breaks=c(2000,2010,2020))+
...
stock.bm.plt <- ggplot(bm.best) + geom_line(aes(x=Year,y=bm.stock,group = Stock,color=spec.tl),linewidth=2) + 
                     facet_wrap(~troph.cat) + scale_x_continuous(name="",labels = c(2000,2010,2020),breaks=c(2000,2010,2020))+
...
## Finding equation of the line between low and high thresholds for each stock ##
  ...
  #find eqn components
    ...
    #CAN'T DO THIS WITH TWO POINTS - NEED TO THINK OF SOMETHING ELSE
