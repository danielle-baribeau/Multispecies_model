...
#test for the different stocks
reg.res <- data.frame(stock = rep("Null", length(c(L.stocks,M.stocks))), 
                      intercept = rep(0, length(c(L.stocks,M.stocks))), 
                      slope = rep(0, length(c(L.stocks,M.stocks))),
                      r.square = rep(0, length(c(L.stocks,M.stocks))), 
                      p  = rep(0, length(c(L.stocks,M.stocks))))
count <- 0
s.p.lam <- NULL
for (d in c(L.stocks, M.stocks)){
  count <- count + 1
  s.lam <- p.lambdas[p.lambdas$stock == d,]
  max.bm <- max(s.lam$year.minus.one)
  s.lam$year.minus.one <- s.lam$year.minus.one/max.bm
  colnames(s.lam)[colnames(s.lam) == "year.minus.one"] <- "prop.bm"
  lam.reg <- lm(log(lambda) ~ prop.bm, data = s.lam)
  reg.res$stock[count] <- d
  reg.res$intercept[count] <- lam.reg$coefficients[1]
  reg.res$slope[count] <- lam.reg$coefficients[2]
  lam.reg <- summary(lam.reg)
  reg.res$r.square[count] <- lam.reg$adj.r.squared
  reg.res$p[count] <- lam.reg$coefficients[2,4]
...
ggplot(s.p.lam) + geom_point(aes(x=prop.bm, y = log(lambda), group = stock)) +
  geom_smooth(aes(x = prop.bm, y = log(lambda), group = stock), method = "lm", formula = y ~ x) +
  labs(x = "Biomass (millions)", y = "Lambda") +
  facet_wrap(~stock, scale = "free", ncol = 5)
#standard error is generally higher at higher biomasses
...
mn.prop.stock.fg.bm$bm.stock <- mn.prop.stock.fg.bm$bm.stock/1000
mn.prop.stock.fg.bm$bm.bfg <- mn.prop.stock.fg.bm$bm.bfg/1000
...
for (m in mn.stock.fg$stock){
  #browser()
  mn <- median(mn.prop.stock.fg.bm$new.prop.bm.stock.bfg[mn.prop.stock.fg.bm$species == m])
...
for (d in c(L.stocks, M.stocks)){
  ...
  pred.tmp <- data.frame(lambda = NA,bm.prop = seq(0,1,by=0.0001),Stock = d)
  if(p.val < 0.2)
  {
    # Now get the appropriate lambda at each proportion of biomass
    pred.tmp$lambda <- exp(predict(mod.tmp,newdata = pred.tmp))
    # I don't know if this is appropriate to use as a sd, but it seems to capture things pretty well.
    pred.tmp$sd <- summary(mod.tmp)$sigma#/sqrt(nrow(tmp))
  } else {
    # If the model didn't fit, I just take the mean of the lambdas on the log scale, so no density dependence below K for these
    pred.tmp$lambda <- exp(mean(log(s.lam$lambda),na.rm=T)) 
    # Again, this is a SD, whether apprprioate I'm unsure, but should be similar scale at least to the above sigma.  Seems to work OK for my data.
    pred.tmp$sd <- sd(log(s.lam$lambda),na.rm=T)#/sqrt(nrow(tmp))
...
for (i in 1:n.sims){
  ...
  prop.sim.K.L <-inv.logit(arima.sim(model =list(ar = c(L.com.prop.cor.lag.1)),n = n.yrs.proj + 1,
                                     ...
                                     innov = c(0,rnorm(n.yrs.proj,0,sd.L.sim.logit/2))) + mn.L.sim.logit)[2:(n.yrs.proj + 1)]
  ...
  for (t in 1:n.yrs.proj){
    ...
    if (t == 1){
      ...
      #play with how many years you include in this mean
      #also look at proportions of each stock in their FGs to see which species dominate
      
      
      #L.multinom.year <- rmultinom(1, bm.sim.K.L[t], 
      #prob = (res.L.stock.df[[1]]$prop.stock.fg.bm[res.L.stock.df[[1]]$year == 2017]))
      rm.bm.sim.K.L <- bm.sim.K.L[t]/1000
      #multicount.l <- 0
      #browser()
      #if (rm.bm.sim.K.L > 1e9){
      # while (rm.bm.sim.K.L > 1e9){
      #  rm.bm.sim.K.L/10
      #  multicount.l <- multicount.l + 1
      #  print(multicount.l)
      # } 
    ...
    if (t > 1) {
      ...
      res.L.bm.last.yr$stock.K <- res.L.bm.last.yr$stock.K/1000
      res.L.bm.last.yr$fg.K <- res.L.bm.last.yr$fg.K/1000
    ...
    for(s in L.stocks)
    {
      ...
      if (bm.last.yr <= cur.K){
        ...
        lam.sd <- log(pred.lam$sd[pred.lam$bm.prop == bm.p & pred.lam$Stock == s])/2
      ...
      if(bm.last.yr > cur.K) 
      {
        lam.med <- 1/median(pred.lam$lambda[pred.lam$Stock == s])
        lam.sd <- median(pred.lam$sd[pred.lam$Stock == s])/2
        
        lam.samp <- rlnorm(1,log(lam.med),log(lam.sd))
      ...
      if (t == 1) {
        ...
        res.L.stock.df[[i]]$lam.med[res.L.stock.df[[i]]$stock == s] <- lam.med
      ...
      if (t > 1){
        ...
        res.L.stock.df[[i]]$lam.med[res.L.stock.df[[i]]$year == t + 2016 &
                                      res.L.stock.df[[i]]$stock == s] <- lam.med
    ...
    for(s in M.stocks)
    {
      ...
      if (bm.last.yr <= cur.K){
        bm.prop <- round(bm.last.yr/cur.K, digits = 4)
        lam.mn <- pred.lam$lambda[pred.lam$bm.prop == bm.prop & pred.lam$Stock == s]
        #lam.sd <- pred.lam$sd[pred.lam$bm.prop == bm.prop & pred.lam$Stock == s]
        lam.sd <- 0.1
        #lam.samp <- rlnorm(1,log(lam.mn),lam.sd)
      ...
      {
        lam.med <- 1/median(pred.lam$lambda[pred.lam$Stock == s])
        #lam.sd <- median(pred.lam$sd[pred.lam$Stock == s])
        lam.sd <- 0.1
        lam.samp <- rlnorm(1,log(lam.med),lam.sd)
        
        #lam.med <- median(na.omit(stock.lambdas$lambda[stock.lambdas$lambda < 1]))
        #lam.sd <- 0.2
        #lam.sd <- sd(log(stock.lambdas$lambda[high.bm.years]),na.rm=T)/2
        #lam.samp <- rlnorm(1,log(lam.med),lam.sd)
      ...
      if (t == 1) {
        ...
        res.M.stock.df[[i]]$lam.med[res.M.stock.df[[i]]$stock == s] <- lam.med
      ...
      if (t > 1){
        ...
        res.M.stock.df[[i]]$lam.med[res.M.stock.df[[i]]$stock == s &
                                      res.M.stock.df[[i]]$year == t + 2016] <- lam.med
...
ggplot(quants[quants$fg == "L",]) +geom_line(aes(x=year, y=L.50/1000000, group=stock,), colour = "grey") +
...
ggplot(quants[quants$fg == "M",]) +geom_line(aes(x=year, y=L.50/1000000, group=stock,), colour = "grey") +
  geom_line(aes(x = year, y = U.50/1000000, group = stock), colour = "grey") +
  geom_line(aes(x = year, y = med/1000000, group = stock)) +
