...
start.com.diff <- start.com.sim - max.com.sim
...
start.L.com.prop.diff.logit <- start.L.com.prop.logit - max.L.sim.logit
...
for (i in 1:n.sims){
  ...
  for (t in 1:n.yrs.proj){
    ...
    for(s in L.stocks)
    {
      ...
      #low.vs.high.bm <- low.vs.high * (0.5*(cur.K))
      ...
      if (t == 1) {
        ...
        res.L.stock.df[[i]]$fg.K[res.L.stock.df[[i]]$stock == s] <- M.K.real
      ...
      if (t > 1){
        ...
        res.L.stock.df[[i]]$fg.K[res.L.stock.df[[i]]$year == t + 2016 &
                                   res.L.stock.df[[i]]$stock == s] <- M.K.real
   ...
   # LP.res$prop.bm.K[LP.res$year == t + 2017] <- LP.res$LP.bm[LP.res$year == t+ 2017]/LP.res$LP.K[LP.res$year == t + 2016]
    ...
    for(s in M.stocks)
    {
      ...
      if(method != "sample")
      {
        ...
        #}
        if(bm.last.yr < low.vs.high.bm)
          #don't use 0.5*cur.K here because it traps larges - they don't get opportunity to grow and mediums go wild
        {
          if(length(low.bm.years) >0)
          {
            lam.med <- median(stock.lambdas$lambda[low.bm.years],na.rm=T)
        ...
        if(bm.last.yr >= low.vs.high.bm & bm.last.yr < cur.K) 
        {
          ...
          lam.sd <- sd(log(stock.lambdas$lambda[high.bm.years]),na.rm=T)/2
          lam.samp <- rlnorm(1,log(lam.med),lam.sd)
          while(lam.samp < 0.5) lam.samp <- rlnorm(1,log(lam.med),lam.sd)
...
ggplot(quants[quants$fg == "L" & quants$year != 2017,]) +geom_line(aes(x=year, y=L.50/1000000, group=stock,), colour = "grey") +
...
ggplot(quants[quants$fg == "M"& quants$year != 2017,]) +geom_line(aes(x=year, y=L.50/1000000, group=stock,), colour = "grey") +
  geom_line(aes(x = year, y = U.50/1000000, group = stock), colour = "grey") +
  geom_line(aes(x = year, y = med/1000000, group = stock)) +
