...
for (i in 1:n.sims){
  ...
  for (t in 1:n.yrs.proj){
    ...
    M.K.real <- bm.sim.K.M[t]/LP.res$prop.bm.K[LP.res$year == t + 2017]
...
########################################################
#lambda and biomass
ggplot(na.omit(res.sim.df[res.sim.df$stock == "Atlantic cod",])) + geom_point(aes(x=bm.stock/1000000, y=lambda))+
  ggtitle(paste("Sim - ",unique(res.sim.df[res.sim.df$stock == "Atlantic cod",]$stock))) +
  labs(x = "Biomass (millions)", y = "Lambda")

ggplot(na.omit(lambdas[lambdas$common == "Atlantic cod",])) + geom_point(aes(x=year.minus.one/1000000, y=lambda))+
  ggtitle(paste("Inputs - ",unique(lambdas[lambdas$common == "Atlantic cod",]$common))) +
  labs(x = "Biomass (millions)", y = "Lambda")
...
ggplot(stock.eco.K) +geom_line(aes(x=year, y=bm.stock, group=sim, colour = sim)) +
