...
for (i in 1:n.sims){
  ...
  bm.sim.K.com <- data.frame(bm = c(arima.sim(model =list(ar = com.cor.lag.1),n = n.yrs.proj + 1,n.start=1,start.innov = start.com.diff/com.cor.lag.1,
                                              innov = c(0,rnorm(n.yrs.proj,0,sd.com.sim/2))) + med.com.sim)[2:(n.yrs.proj + 1)],
                             Years = 1:n.yrs.proj,sim = i)
  ...
  prop.sim.K.L <-inv.logit(arima.sim(model =list(ar = c(L.com.prop.cor.lag.1)),n = n.yrs.proj + 1,
                                     ...
                                     innov = c(0,rnorm(n.yrs.proj,0,sd.L.sim.logit/2))) + med.L.sim.logit)[2:(n.yrs.proj + 1)]
  ...
  for (t in 1:n.yrs.proj){
    ...
    if (t == 1){
      ...
      rm.bm.sim.K.L <- bm.sim.K.L[1]/1000
    ...
    for(s in L.stocks)
    {
      ...
      if (bm.last.yr <= cur.K){
        ...
        #lam.sd <- 0.1
        lam.sd <- pred.lam$sd[pred.lam$bm.prop == bm.p & pred.lam$Stock == s]/2
        lam.samp <- rlnorm(1,log(lam.mn), lam.sd)
        #while(lam.samp > max(s.p.lam$lambda[s.p.lam$stock == s])) lam.samp <- rlnorm(1,log(lam.med),lam.sd)
        bm.p <- NULL
      }
      ...
      if(bm.last.yr > cur.K) 
      {
        ...
        while(lam.samp > 1.05) lam.samp <- rlnorm(1,log(lam.mn),lam.sd)
...
#[ & res.sim.df$sim == 11,]
ggplot(na.omit(res.sim.df[res.sim.df$fg == "L",])) +
...
ggplot(na.omit(res.sim.df[res.sim.df$fg == "M",])) +
  geom_line(aes(x = year, y = stock.K/1000000, group = sim), colour = "lightblue") +
  geom_line(aes(x = year, y = bm.stock/1000000, group = sim)) +
