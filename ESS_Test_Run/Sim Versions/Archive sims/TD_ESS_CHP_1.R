...
#NEW: placeholder input data for conversion between H/P removals and other species removals
  #assuming for now that there is difference between conversion for H and P
  #check with H and D if H and P should be combined - same gear?
conv <- data.frame(stock = c(L.stocks, M.stocks), conv.h = c(1,1,rep(0.001, 13)),
                   conv.p = c(1,1,rep(0.001, 13)))
#to get conversions, use data from WSS (where CHPs is currently happening)?
...
#I think this works; longfin hake is just getting hammered!
for (i in 1:n.sims){
  ...
  for (t in 1:n.yrs.proj){
    ...
    for(s in L.stocks)
    {
      ...
      #ONLY FISHING HADDOCK AND POLLOCK
      #NEW: getting removals for H and P, and bycatch for everything else
      #NOTE: if you fish too strong, get negative biomasses in some species (conversion factors matter!)
      #if (s == "White hake") browser()
      if (s %in% c("Pollock", "Haddock")){
        ex.curr <- 0.1
        removals <- bm.last.yr * ex.curr
        if (s == "Pollock") rem.p <- removals
        if (s == "Haddock") rem.h <- removals
      }else {
        conv.h <- conv$conv.h[conv$stock == s]
        conv.p <- conv$conv.p[conv$stock == s]
      rem.conv.h <- rem.h * conv.h
      rem.conv.p <- rem.p * conv.p
      removals <- rem.conv.h + rem.conv.p
      }
    ...
    for(s in M.stocks)
    {
      ...
      #NEW: fishing mediums based on H and P removals
      #if (s == "Silver hake") browser()
        removals <- NULL
        conv.h <- conv$conv.h[conv$stock == s]
        conv.p <- conv$conv.p[conv$stock == s]
        rem.conv.h <- rem.h * conv.h
