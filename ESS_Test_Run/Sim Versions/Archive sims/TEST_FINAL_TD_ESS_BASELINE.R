...
for (d in c(L.stocks, M.stocks)){
  ...
  med.bm <- median(s.lam$year.minus.one)
  mn.bm <- mean(s.lam$year.minus.one)
  #like median better because less influence from big spikes
  s.lam$bm.prop <- s.lam$year.minus.one/med.bm
  ...
  #p.val <- summary(mod.tmp)$coefficients[2,4]
    #just used p-value as check; relationship was always strong so used linear mod for all stocks
  ...
  pred.tmp$sd <- summary(mod.tmp)$sigma #This is NOT residual standard error! RDocumentation says so
    #this is actually SD
  #r.square <- summary(mod.tmp)$r.square
    #just used this as QC check
...
for (i in 1:n.sims){
  ...
  #innov = c(1,rlnorm(n.yrs.proj-1,0,se.com.sim))*mn.com.sim - mn.com.sim) + mn.com.sim),
  ...
  #bm.sim.K.com <- data.frame(bm = c(arima.sim(model =list(ar = com.cor.lag.1),n = n.yrs.proj,n.start=1,start.innov = start.com.diff/com.cor.lag.1,
  #innov = c(0,rnorm(n.yrs.proj-1,0,se.com.sim/2))) + mn.com.sim),
  #Years = 1:n.yrs.proj,sim = i)
