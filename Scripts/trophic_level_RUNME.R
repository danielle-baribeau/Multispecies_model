...
# Try the function
exploit.mn <- data.frame(ex.mn = c(1,0,rep(0.1,length(stock.lst)-2)),Stock = names(stock.lst))
exploit.sd <- data.frame(ex.sd = c(0,0,rep(0.1,length(stock.lst)-2)),Stock = names(stock.lst))
