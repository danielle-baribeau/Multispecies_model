...
#get transfer efficiency between M and L
t.eff <- data.frame(year = c(2000:2017), bm.com = com.bm, L.bm = unique(bm.best.b$bm.bfg[bm.best.b$BFG == "L"]),
                    M.bm = unique(bm.best.b$bm.bfg[bm.best.b$BFG == "M"]))
t.eff$M.prop.com <- t.eff$M.bm/t.eff$bm.com
t.eff$L.prop.com <- t.eff$L.bm/t.eff$bm.com
t.eff$te <- t.eff$L.prop.com/t.eff$M.prop.com

#avg.te <- mean(t.eff$te)
avg.te <- 1
#going with mean because it gives higher # and we expect the te to be closer to 1 as stocks are at very similar TLs
...
for (i in 1:n.sims){
  ...
  for (t in 1:n.yrs.proj){
    ...
    if (t == 1) res.M.stock.df[[i]]$delta.K.fg[res.M.stock.df[[i]]$year == 2017] <- 0
    
    if (t > 1) res.M.stock.df[[i]]$delta.K.fg[res.M.stock.df[[i]]$year == t + 2016] <- M.K - res.M.stock.df[[i]]$fg.K[res.M.stock.df[[i]]$year == t + 2015]
    ...
    L.K.real <- L.K.real + ((unique(res.M.stock.df[[i]]$delta.K.fg[res.M.stock.df[[i]]$year == t + 2016]))*avg.te)
    ...
    for(s in L.stocks)
    {
      ...
      #reduce K of LPs based on proportion of M K that Ms used
      #if (s %in% LP.stocks){
      #cur.K <- cur.K * M.bm.prop.K
      #print("LP")
      #} else {cur.K <- cur.K
      #print("LB")
      #}
    ...
    L.K <- sum(res.L.stock.df[[i]]$stock.K[res.L.stock.df[[i]]$year == t + 2016])

    if (t == 1) res.L.stock.df[[i]]$delta.K.fg[res.L.stock.df[[i]]$year == 2017] <- 0
    
    if (t > 1) res.L.stock.df[[i]]$delta.K.fg[res.L.stock.df[[i]]$year == t + 2016] <- L.K - res.L.stock.df[[i]]$fg.K[res.L.stock.df[[i]]$year == t + 2015]
...
bm.com.test <- res.sim.df[,c(1,2,9)]
...
for (d in c(L.stocks, M.stocks)){
  ...
  for (x in 1:n.sims){
    ...
    s.sim.start <- mn.in$mean[mn.in$stock == d]
...
for (d in c(L.stocks, M.stocks)){
  ...
  #print(paste(summary(s.res$lambda)))
    s.sim.start <- mn.in$mean[mn.in$stock == d]
    s.sim.end <- quants$med[quants$stock == d & quants$year == 2046]
    s.sim.diff <- s.sim.start - s.sim.end
    s.sim.per <- (s.sim.diff/s.sim.start)*100
    s.sim.per <- s.sim.per*-1
    print(paste(d))
    print(paste(s.sim.per))
  }
