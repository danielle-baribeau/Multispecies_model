...
start.M.com.prop.diff.logit <- start.M.com.prop.logit - mn.M.sim.logit
...
#NEW: added in a "do we want to fish either functional group?" input
  #need to change because we only want to fish dominant group
fish.L <- "NO"
fish.M <- "NO"
...
for (d in c(L.stocks, M.stocks)){
  ...
  #print(paste(min.lam))
}#end lambda regression loop

plot.text <- data.frame(stock = s.p.lam$stock,
      p = paste("p-value:", round(s.p.lam$p, digits = 4), "; r-square:", round(s.p.lam$r, digits = 2)))
plot.text <- unique(plot.text)

ggplot(s.p.lam[s.p.lam$stock %in% L.stocks,], aes(x = bm.prop, y = log(lambda))) + geom_point() +
  geom_smooth(method = "lm") +
  labs (x = "Proportion of median historical biomass", y = "Lambda (log-transformed)") +
  facet_wrap(~stock, ncol = 4, scale = "free") +
  geom_text(data = plot.text[plot.text$stock %in% L.stocks,],
            mapping = aes(x = -Inf, y = -Inf, label = p),
            hjust   = -0.05,
            vjust   = -1,
            size = 3)

ggplot(s.p.lam[s.p.lam$stock %in% M.stocks,], aes(x = bm.prop, y = log(lambda))) + geom_point() +
  geom_smooth(method = "lm") +
  labs (x = "Proportion of median historical biomass", y = "Lambda (log-transformed)") +
  facet_wrap(~stock, ncol = 4, scale = "free") +
  geom_text(data = plot.text[plot.text$stock %in% M.stocks,],
            mapping = aes(x = -Inf, y = -Inf, label = p),
            hjust   = -0.05,
            vjust   = -1,
            size = 3)
...
#### NEW ####
#Set up management strategy for each stock
# Modified from DK's code #
#if no HCR, default is to fish at historical removal rate (exploitation rate from time series)
#if HCR, LRP is 40% of median and USR is median; no fishing below LRP and fishing decreases linearly between LRP and USR
#above USR, fishing exploitation rate is 0.3

#DK made .csv with everything here in it; then just change .csv when needed!
ex.dat <- NULL

#no stock-specific management data yet, so all stocks will get treated the same
#when removals data are available, set mgmt plan outside of this file
#this is destined for .CSV
#Just do strainght-up exploitation rates - no fm
for (f in c(L.stocks, M.stocks)){
  #currently in relation to historical biomass - change to historical removal rates later
  ex.dat[[f]] <- data.frame(stock = f, lrp = NA, usr = NA, rr = NA, er.mn = NA, er.sd = NA,
                            fm.mn = NA, er.below.lrp = NA, use.hcr = NA)
  #set lrp to 40% of median historical biomass
  ex.dat[[f]]$lrp <- 0.4*median(p.lambdas$pop.bm[p.lambdas$stock == f], na.rm = T)
  #set usr to median historical biomass
  ex.dat[[f]]$usr <- median(p.lambdas$pop.bm[p.lambdas$stock == f], na.rm = T)
  #using placeholders here for now to get removal rate - set to median historical fishing mortality
  #ex.dat[[f]]$rr <- 1-exp(-median(bm.best$er[bm.best$Stock == f],na.rm=T)), this is set as instantaneous
  
  #rr = removal ref point - don't want to go above 0.15 in terms of exploitation
  ex.dat[[f]]$rr <- 0.15
  
  #set mean exploitation rate to be rr (when available)
  #CHECK - Why this relationship?
  ### going from instantaneous rate to proportion here ###
  ex.dat[[f]]$er.mn <- -log(1-ex.dat[[f]]$rr)
  #CHECK - define removal rate and fishing mortality differences
  ex.dat[[f]]$fm.mn <- ex.dat[[f]]$rr
  #default HCRs - no fishing below lrp and 0.3 above usr
  ex.dat[[f]]$er.below.lrp <- 0
  #set sd for er sampling
  ex.dat[[f]]$er.sd <- ex.dat[[f]]$er.mn/2
  #currently not using HCR for any stocks
  ex.dat[[f]]$use.hcr <- "YES"
}#end management loop
...
for (i in 1:n.sims){
  ...
  prop.sim.K.M <-inv.logit(arima.sim(model =list(ar = c(M.com.prop.cor.lag.1)),n = n.yrs.proj + 1,
                                     ...
                                     innov = c(0,rnorm(n.yrs.proj,0,sd.M.sim.logit))) + mn.M.sim.logit)[2:(n.yrs.proj + 1)]
  ...
  for (t in 1:n.yrs.proj){
    ...
    if (t == 1){
      res.L.stock.df[[i]] <- data.frame(sim = i, year = 2017,
                                        ...
                                        removals = NA, 
                                        er = NA,
                                        lambda = NA, stock.K = NA, fg.K = NA, com.K = NA, multinom = NA,
                                        lam.med = NA,
                                        status = NA,
                                        stat.num = NA)
    ...
    if (t > 1) {
      ...
      res.M.bm.last.yr <- res.M.bm.last.yr[,c(1:4,7,12:13)]
    ...
    for(s in M.stocks)
    {
      ...
      if (fish.M == "YES"){
        fishing <- proj.catch(dat = ex.dat[[s]], bm = bm.last.yr)
        er <- fishing$er
        status <- fishing$status
        stat.num <- fishing$stat.num
      }else{
        er <- 0
        status <- "No fishing"
        stat.num <- 4
      }
      #CHECK - didn't see following step in DK's code; couldn't find where tst.res was getting calculated
      removals <- bm.last.yr * er
      ...
      if (t == 1) {
        ...
        res.M.stock.df[[i]]$er[res.M.stock.df[[i]]$stock == s] <- er
        ...
        res.M.stock.df[[i]]$status[res.M.stock.df[[i]]$stock == s] <- status
        res.M.stock.df[[i]]$stat.num[res.M.stock.df[[i]]$stock == s] <- stat.num
      ...
      if (t > 1){
        ...
        res.M.stock.df[[i]]$er[res.M.stock.df[[i]]$year == t + 2016 &
                                 res.M.stock.df[[i]]$stock == s] <- er
        ...
        res.M.stock.df[[i]]$status[res.M.stock.df[[i]]$year == t + 2016 &
                                     res.M.stock.df[[i]]$stock == s] <- status
        res.M.stock.df[[i]]$stat.num[res.M.stock.df[[i]]$year == t + 2016 &
                                       res.M.stock.df[[i]]$stock == s] <- stat.num
    ...
    if (t > 1) {
      ...
      res.L.bm.last.yr <- res.L.bm.last.yr[,c(1:4,7,1:4,7,12:13)]
    ...
    for(s in L.stocks)
    {
      ...
      if (t == 1) {
        ...
        res.L.stock.df[[i]]$er[res.L.stock.df[[i]]$stock == s] <- er
        ...
        res.L.stock.df[[i]]$status[res.L.stock.df[[i]]$stock == s] <- status
        res.L.stock.df[[i]]$stat.num[res.L.stock.df[[i]]$stock == s] <- stat.num
      ...
      if (t > 1){
        ...
        res.L.stock.df[[i]]$er[res.L.stock.df[[i]]$year == t + 2016 &
                                       res.L.stock.df[[i]]$stock == s] <- er
        ...
        res.L.stock.df[[i]]$status[res.L.stock.df[[i]]$year == t + 2016 &
                                     res.L.stock.df[[i]]$stock == s] <- status
        res.L.stock.df[[i]]$stat.num[res.L.stock.df[[i]]$year == t + 2016 &
                                       res.L.stock.df[[i]]$stock == s] <- stat.num
