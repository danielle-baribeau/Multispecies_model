...
#if using historical removals in projections, load data
...
for (i in 1:n.sims){
  ...
  for (t in 1:n.yrs.proj){
    ...
    for(s in M.stocks)
    {
      ...
      #if using historical removals
      removals <- rem.sum$med.rem[rem.sum]
    ...
    for(s in L.stocks)
    {
      ...
      if (t > 1){
        ...
        res.L.stock.df[[i]]$lam.med[res.L.stock.df[[i]]$stock == s &
                                      res.L.stock.df[[i]]$year == t + 2016] <- NA
  ...
  #browser()
  prop.K.L[[i]] <- data.frame(sim = i, year = 1:n.yrs.proj, prop.com.M.K = c(prop.sim.K.M))
  com.sim[[i]] <- data.frame(sim = i, year = 1:n.yrs.proj, com.K = c(bm.sim.K.com$adj.bm))
...
#are proportions summing to 1 each year?
prop <- na.omit(res.sim.df) |> collapse::fgroup_by(fg, year, sim) |> collapse::fsummarize(sum.prop = sum(prop.stock.fg.bm)) |> ungroup()

prop.L <- res.sim.df[res.sim.df$fg == "L",]
prop.L <- prop.L |> collapse::fgroup_by(year, sim) |> collapse::fsummarize(sum.prop = sum(prop.stock.fg.bm)) |> ungroup()

prop.M <- res.sim.df[res.sim.df$fg == "M",]
prop.M <- prop.M |> collapse::fgroup_by(year, sim) |> collapse::fsummarize(sum.prop = sum(prop.stock.fg.bm)) |> ungroup()
...
ggplot(p.lambdas) +
  ...
  geom_smooth(aes(x = year, y = lambda), method = "lm") +
...
ggplot(hist.com.bm) + geom_line(aes(x = year, y = com.bm/1000000)) +
  ...
  geom_line(data = na.omit(res.sim.df), aes(x = year, y = com.K/1000000, group = sim), , colour = "lightblue") +
  geom_hline(yintercept = (mean(na.omit(res.sim.df$com.K)))/1000000, colour = "lightblue") +
  #geom_line(data = na.omit(res.sim.df), aes(x = year, y = bm.com/1000000, group = sim), , colour = "darkblue") +
  #geom_hline(yintercept = (mean(na.omit(res.sim.df$bm.com)))/1000000, colour = "darkblue") +
  labs(x = "Year", y = "Community biomass (K in lightblue) \n(millions of kg)") +
  theme(axis.title = element_text(face = "bold", size = 20))
